...
vec3 applyColorPalette(vec3 color, int palette) {
  if (palette == 1) { // Sage Green (matching Claru theme)
    float lum = dot(color, vec3(0.299, 0.587, 0.114));
    // Sage green: hue ~100, low saturation
    return vec3(lum * 0.57, lum * 0.69, lum * 0.56);
...
float getChar(float brightness, vec2 p, int style) {
  ...
  } else if (style == 1) { // Dense - more character detail
    if (brightness < 0.1) val = 0.0;
    else if (brightness < 0.2) val = (grid.x == 1.0 && grid.y == 1.0) ? 0.5 : 0.0;
    else if (brightness < 0.3) val = ((grid.x == 1.0 || grid.x == 2.0) && grid.y == 1.0) ? 1.0 : 0.0;
    else if (brightness < 0.4) val = (grid.y == 1.0 || grid.y == 2.0) ? 0.8 : 0.0;
    else if (brightness < 0.5) val = mod(grid.x + grid.y, 2.0) == 0.0 ? 1.0 : 0.2;
    else if (brightness < 0.6) val = (grid.x == 0.0 || grid.x == 3.0 || grid.y == 0.0 || grid.y == 3.0) ? 1.0 : 0.4;
    else if (brightness < 0.7) val = (grid.x == 1.0 || grid.x == 2.0) ? 1.0 : 0.6;
    else if (brightness < 0.8) val = 0.8;
    else val = 1.0;
